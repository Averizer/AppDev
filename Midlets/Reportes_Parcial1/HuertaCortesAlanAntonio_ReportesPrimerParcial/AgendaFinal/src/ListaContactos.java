...
public class ListaContactos extends List implements CommandListener {
    ...
    public ListaContactos(Principal mid){
        super("Contactos",Choice.EXCLUSIVE);
        p = mid;
        register = new Command("Registrar", Command.SCREEN, 1); 
        delete = new Command("Eliminar", Command.SCREEN, 2); 
        edit = new Command("Editar", Command.SCREEN, 3); 
        addCommand(register); 
    ...
    public void constructList(){
        ...
        for(int i=1;i<=rmso.rs.getNumRecords();i++){
            ...
            if(eliminados.indexOf(find) == -1){
                ...
                append(datos[1]+" "+datos[2],imagen); // cantidad de campos
            }                     
        }
        rmso.cerrar();
        }catch(Exception e){
